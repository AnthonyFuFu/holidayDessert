...
VALUES  ('傅勝宏', 'FU830917', '830917', 'm', '0999000000','s9017611@gmail.com','台北市中正區博愛路36號',STR_TO_DATE('1994-09-17','%Y-%m-%d'),1,1,NULL),
		('阿勝', 'ASHENG', '830917', 'f', '0988000000','s9017622@gmail.com','台北市中正區博愛路36號',STR_TO_DATE('1994-09-17','%Y-%m-%d'),1,1,NULL),
        ('傅', 'FU30917', '830917', 'm', '0999000000','s9017633@gmail.com','台北市中正區博愛路36號',STR_TO_DATE('1994-09-17','%Y-%m-%d'),1,1,NULL),
        ('勝', 'FU83917', '830917', 'm', '0999000000','s9017644@gmail.com','台北市中正區博愛路36號',STR_TO_DATE('1994-09-17','%Y-%m-%d'),1,1,NULL),
        ('宏', 'FU83097', '830917', 'm', '0999000000','s9017655@gmail.com','台北市中正區博愛路36號',STR_TO_DATE('1994-09-17','%Y-%m-%d'),1,1,NULL)
        ;
...
CREATE TABLE chat_room(
    ...
    ROOM_LAST_UPDATE DATETIME DEFAULT NOW() COMMENT '聊天室最後更新時間'
) COMMENT='聊天室';
...
CREATE TABLE ticket (
    ...
    TICKET_STATUS INT(2) NOT NULL DEFAULT 0 COMMENT '活動狀態(0:尚未開始 1:開始期間 2:活動結束 3:活動取消)',
...
VALUES ('五月天演唱會', '2025-07-10 19:00:00', CONCAT(CURDATE(), ' 10:00:00'), CONCAT(DATE_ADD(CURDATE(), INTERVAL 1 WEEK), ' 23:59:59'), 1150, 1),
       ('小巨蛋動漫展', '2025-08-05 10:00:00', CONCAT(CURDATE(), ' 09:00:00'), CONCAT(DATE_ADD(CURDATE(), INTERVAL 1 WEEK), ' 23:59:59'), 1150, 2),
       ('跨年煙火派對', '2025-12-31 22:00:00', CONCAT(CURDATE(), ' 12:00:00'), CONCAT(DATE_ADD(CURDATE(), INTERVAL 1 WEEK), ' 23:59:59'), 1150, 0);
...
CREATE TABLE ticket_type (
    ...
    TYPE_REMAINING INT NOT NULL COMMENT '剩餘票數',
    TYPE_MAX_PER_PERSON INT NOT NULL DEFAULT 4 COMMENT '每人購買上限',
    LOCK_VERSION INT NOT NULL DEFAULT 0 COMMENT '樂觀鎖版本號',
...
INSERT INTO ticket_type (TICKET_ID, TYPE_NAME, TYPE_PRICE, TYPE_QUANTITY, TYPE_REMAINING)
VALUES (1, '搖滾區', 3200, 100, 100),(1, '看台區', 1800, 300, 300),(1, '一般票', 350, 250, 250),(1, '免費票', 0, 500, 500),
	   (2, '搖滾區', 3200, 100, 100),(2, '看台區', 1800, 300, 300),(2, '一般票', 350, 250, 250),(2, '免費票', 0, 500, 500),
       (3, '搖滾區', 3200, 100, 100),(3, '看台區', 1800, 300, 300),(3, '一般票', 350, 250, 250),(3, '免費票', 0, 500, 500);
...
CREATE TABLE ticket_order (
    ...
    TICKET_ORD_STATUS INT(2) NOT NULL DEFAULT 0 COMMENT '訂單狀態(0:待付款 1:已付款 2:已取消 3:退款中 4:已退款 5:逾時取消)',
    TICKET_ORD_TIME DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '下單時間',
    TICKET_ORD_AMOUNT INT NOT NULL COMMENT '實際付款金額（TYPE_PRICE × QUANTITY 快照）',
	TICKET_ORD_EXPIRE DATETIME COMMENT '付款期限（例如：下單後15分鐘）',
	TICKET_ORD_PAY_TIME DATETIME COMMENT '付款時間',
...
INSERT INTO ticket_order (MEM_ID, TYPE_ID, TICKET_ORD_QUANTITY, TICKET_ORD_STATUS, TICKET_ORD_AMOUNT, TICKET_ORD_EXPIRE, TICKET_ORD_PAY_TIME)
VALUES (1, 1, 2, 1, 6400, DATE_ADD(NOW(), INTERVAL 15 MINUTE), NOW()),
       (2, 3, 1, 0, 350, DATE_ADD(NOW(), INTERVAL 15 MINUTE), NULL);
