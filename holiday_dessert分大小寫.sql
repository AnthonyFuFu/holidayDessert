...
CREATE TABLE ticket (
    TICKET_ID INT AUTO_INCREMENT NOT NULL PRIMARY KEY COMMENT '主鍵，自動遞增的票券 ID',
    TICKET_EVENT VARCHAR(100) NOT NULL UNIQUE COMMENT '活動名稱，唯一值',
    TICKET_QUANTITY INT NOT NULL COMMENT '票券剩餘數量',
    TICKET_PRICE DECIMAL(10, 2) NOT NULL COMMENT '票價，兩位小數',
    TICKET_CREATE_TIME TIMESTAMP DEFAULT CURRENT_TIMESTAMP COMMENT '建立時間，預設當前時間',
    TICKET_UPDATE_TIME TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新時間，每次修改時自動更新'
) COMMENT='票券資料表';

INSERT INTO ticket (TICKET_EVENT, TICKET_QUANTITY, TICKET_PRICE)
VALUES 	('五月天演唱會', 100, 3200.00),
		('小巨蛋動漫展', 250, 350.00),
		('跨年煙火派對', 500, 0.00);
