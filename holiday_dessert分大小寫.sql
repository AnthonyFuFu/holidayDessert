...
CREATE TABLE ticket (
    ...
    TICKET_PRICE INT NOT NULL COMMENT '票價',
    TICKET_CREATE_TIME datetime DEFAULT CURRENT_TIMESTAMP COMMENT '建立時間，預設當前時間',
    TICKET_UPDATE_TIME datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新時間，每次修改時自動更新'
) COMMENT='票券資料表';

INSERT INTO ticket (TICKET_EVENT, TICKET_QUANTITY, TICKET_PRICE)
VALUES 	('五月天演唱會', 100, 3200),
		('小巨蛋動漫展', 250, 350),
		('跨年煙火派對', 500, 0);
